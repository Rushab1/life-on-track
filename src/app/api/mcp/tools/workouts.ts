...
export function registerWorkoutTools(
  ...
  server.tool(
    "log_workout",
    "Log or update workout sets for a date. Always batch — pass an array of sets. Each set may be an INSERT (no id) or an UPDATE (id = existing workout_set.id). Notes default to append mode on updates; use notes_mode='write' to replace. PROTOCOL: (1) Elicit sets one at a time — one single-select question per set, options as weight×reps taken from the user's previous session of that exercise via get_history, displayed inline as \"(prev: 45, 50, 50)\". Never use the phrase \"top set\". (2) Do NOT call this tool per set: hold answers and call ONCE per exercise or superset when the user says they are done with it. Put superset pairings, substitutions, and PRs in set notes. (3) Immediately after each batch, run a short pain panel for the body sites that exercise loads (plus baseline-flagged sites) and append it to daily notes via save_day. Exercise names are validated against the exercises table; if any are unknown the entire request is rejected with suggestions. If an exercise name is rejected, call manage_exercise action=\"list\" and look for near-matches (e.g. \"Supine Leg Lifts\" vs \"Supine Leg Raises\") before creating anything new. Follow the workout-logging prompt for the full session protocol.",
    {
